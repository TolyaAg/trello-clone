...
class List extends Component {
    ...
    render() {
        ...
        const items = tasks.map((task) => (
            <ListItem key={task.name} {...task} deleteItem={this.deleteItem} changeItemStatus={this.changeItemStatus}/>
        ))
        return (
            <div className="List">
                <div className="List__title">
                    {name}
                    <button className="List__delete-button" onClick={this.deleteList}>+</button>
                </div>
                <hr width='100%'/>
                <Input className="List__input" value={taskName} onChange={this.inputChange} onKeyPress={this.inputKey}/>
                {items}
            </div>
        )
    }
